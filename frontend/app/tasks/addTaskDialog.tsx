...
export default function AddTaskDialog({ projectList, taskList }: any) {
                ...
                <SelectContent>
                  {projectList &&
                    projectList.map((project: any) => (
                      <SelectItem value={project.project_id}>
                        {project.name}
                      </SelectItem>
                    ))}
                </SelectContent>
              ...
              <div className="relative">
                ...
                <div className="flex flex-col gap-2">
                  <label
                    ...
                    className="block text-sm font-medium"
                  >
                    Estimated Effort <span className="text-red-500">*</span>
                  </label>
                  <Select
                    onValueChange={(val) => setEstimatedEffort(parseInt(val))}
                  >
                    <SelectTrigger className="w-[180px]">
                      <SelectValue placeholder="Select effort (1-5)" />
                    </SelectTrigger>
                    <SelectContent>
                      {[1, 2, 3, 4, 5].map((val) => (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="relative w-full">
                      <button
                        ...
                        className="w-full border rounded-md px-3 py-2 text-left shadow focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                      >
                        {dependsOn.length > 0
                          ? `${dependsOn.length} task(s) selected`
                          : "Select dependencies"}
                      </button>

                      {dependsDropdownOpen && (
                        <div className="absolute z-50 w-full mt-2 bg-stone-800 border border-gray-300 rounded-md max-h-60 overflow-auto">
                          {taskList && taskList
                            // .filter(
                            //   (task: any) => task.task_id !== taskId
                            // ) // Optional: avoid self-dependency
                            .map((task: any) => {
                              const selected = dependsOn.includes(task.task_id);
                              return (
                                <div
                                  key={task.task_id}
                                  onClick={() => {
                                  ...
                                  className={`cursor-pointer px-4 py-2 flex items-center justify-between hover:bg-stone-700 ${
                    ...
                    {/* Show selected task names below */}
                    {dependsOn.length > 0 && (
